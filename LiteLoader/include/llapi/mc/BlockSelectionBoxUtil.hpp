/**
 * @file  BlockSelectionBoxUtil.hpp
 *
 */
#pragma once
#define AUTO_GENERATED
#include "llapi/Global.h"

#define BEFORE_EXTRA

#undef BEFORE_EXTRA


namespace BlockSelectionBoxUtil {

#define AFTER_EXTRA

#undef AFTER_EXTRA
    /**
     * @symbol ?assertAABBOriginAndSizeValid\@BlockSelectionBoxUtil\@\@YA_NAEBVVec3\@\@0\@Z
     */
    MCAPI bool assertAABBOriginAndSizeValid(class Vec3 const &, class Vec3 const &);
    /**
     * @symbol ?computeAABBFromOriginAndSize\@BlockSelectionBoxUtil\@\@YA?AVAABB\@\@AEBVVec3\@\@0\@Z
     */
    MCAPI class AABB computeAABBFromOriginAndSize(class Vec3 const &, class Vec3 const &);

};